...
:::warning[Beware when testing]
To work, [splitComponent()] needs to know the assets generated by Webpack
for each split chunk. In real applications relying on this library, these data
are automatically injected (i) on the server-side by [launchServer()] _via_ SSR
context; (ii) on the client-side by [launchClient()] _via_ a call to
the [setClientChunkGroups()] function.

When testing with Jest, if you use [splitComponent()] inside a render,
and you don't use [launchClient()] / [launchServer()], you should pass
these chunk data manually, otherwise [splitComponent()] will throw if
...
- [v1.16.0] &mdash; multiple [splitComponent()] signature changes:
